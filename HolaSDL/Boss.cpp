...
void Boss::fase1()
{
	...
	{
		transform.position.setX(transform.position.getX()-velocidad*(Time::getInstance()->DeltaTime+1));
	}
	...
	{
		transform.position.setY(transform.position.getY() - velocidad*(Time::getInstance()->DeltaTime + 1));
	}
...
void Boss::fase2()
{
	...
	if ((auxX > 0 && recorridoX < auxX) || auxX < 0 && recorridoX>auxX)
		transform.position.setX(transform.position.getX()+auxVelX*(Time::getInstance()->DeltaTime +1));
	recorridoX += velocidad*(Time::getInstance()->DeltaTime +1);
	...
	if ((auxY > 0 && recorridoY < auxY) || auxY < 0 && recorridoY>auxY)
		transform.position.setY(transform.position.getY() + auxVelY*(Time::getInstance()->DeltaTime + 1));
	recorridoY += velocidad*(Time::getInstance()->DeltaTime + 1);
}
