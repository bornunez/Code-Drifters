...
void MCMovementInput::update()
{
	...
	if (mc->canMove()){
		...
		if (velocity.getX() == 0 && velocity.getY() == 0 && (mc->getMCState() != MCState::Attack && mc->getMCState() != MCState::Hooking && mc->getMCState() != MCState::Shot && mc->getMCState() != MCState::DashEnd)) {
