...
void Animation::setFlip(SDL_RendererFlip flip)//Rota el sprite y con él todas las boxes y puntos
{
	...
	for (uint i = 0; i < animFrames.size(); i++) {
		...
		for (uint j = 0; j < hitboxOffset.size(); j++) {
			...
			hitboxOffset[j].first = frameW*Game::getGame()->getScale() - (hitboxOffset[j].first + hitboxes[j].w);//Desplaza de forma simétrica en x el punto
		...
		for (uint j = 0; j < hurtboxOffset.size(); j++) {
			std::pair<int, int> aux;
			hurtboxOffset[j].first = frameW * Game::getGame()->getScale() - (hurtboxOffset[j].first + hurtboxes[j].w);//Desplaza de forma simétrica en x el punto
		...
		gunPosOffset.first = frameW * Game::getGame()->getScale() - (gunPosOffset.first);
