...
void MCAnimationComponent::handleFullCharge()
{
	int mouseX, mouseY;
	SDL_Point p;
	SDL_Rect r;
	...
	if (angle > 210 && angle <= 270) {//Arriba izquierda
		...
		gameObject->changeCurrentAnimation("FULLCHARGE_TOP");
		...
		gameObject->changeCurrentAnimation("FULLCHARGE_RIGHT");
		gameObject->getCurrentAnimation()->setFlip(SDL_FLIP_HORIZONTAL);
		t->direction.set(-1, 0);
	}
	else if (angle >= 90 && angle < 150) {//Abajo a la izquierda
		gameObject->changeCurrentAnimation("FULLCHARGE_BOT");
		gameObject->getCurrentAnimation()->setFlip(SDL_FLIP_HORIZONTAL);
		t->direction.set(0, 1);
	}
